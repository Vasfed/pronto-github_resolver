...
RSpec.describe Pronto::Formatter::GithubPullRequestReviewFormatter do # rubocop:disable RSpec/FilePath
  ...
  describe "#format" do
    subject(:perform_format) { described_class.new.format(messages, repo, patches) }
    ...
    before do
      ...
      allow_any_instance_of(Pronto::Github).to receive(:fetch_review_threads).and_return(existing_threads)
    end

    it "adds comment" do
      expect_any_instance_of(Octokit::Client).to receive(:create_pull_request_review).once.with(
        "test/test", 10, a_hash_including(event: "COMMENT", comments: [a_hash_including(body: "New message")])
      )
    ...
    context "when previous review was with request changes and all fixed" do
      ...
      it "approves PR" do
        # TODO: try DISMISS?
        expect_any_instance_of(Octokit::Client).to receive(:create_pull_request_review).once.with(
          "test/test", 10, a_hash_including(event: "APPROVE")
        )
